...
#
# Relies on `GIT_CLIFF_CONFIG` is present in env.
# Otherwise, tries to find it in the following guesses:
# - `../cpp-linter-org/.github/cliff.toml`
# - `../.github/.github/cliff.toml`
export def find-cliff-toml [] {
    if ($env | get --optional "GIT_CLIFF_CONFIG" | is-not-empty) {
        ...
        mut result = null
        for guess in [cpp-linter-org .github] {
            let hits = glob $"($parent)/($guess)/.github/cliff.toml"
            if ($hits | is-not-empty) {
                $result = $hits | first
                print $"(ansi yellow)Attempting to use ($result) instead.(ansi reset)"
                break
            }
        }
        $result
